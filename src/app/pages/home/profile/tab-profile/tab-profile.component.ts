...
export class TabProfileComponent implements OnInit {
  src: any;
  updateDataUser: FormGroup = new FormGroup({
    ...
    if (file.size / 1024 <= 1000) {
      if (extn === 'png' || extn === 'jpg' || extn === 'gif' || extn === 'jpeg') {
        ...
        myReader.onloadend = (e) => {
          this.getImagen(myReader.result);
  ...
  updateData() {
    const data = {
      // tslint:disable-next-line:max-line-length
      firstName: String(this.updateDataUser.get('firstName').value) === String(this.firstnameData) ? null : this.updateDataUser.get('firstName').value,
      // tslint:disable-next-line:max-line-length
      lastName: String(this.updateDataUser.get('lastName').value) === String(this.lastnameData) ? null : this.updateDataUser.get('lastName').value,
      // tslint:disable-next-line:max-line-length
      email: String(this.updateDataUser.get('email').value) === String(this.emailData) ? null : this.updateDataUser.get('email').value,
      // tslint:disable-next-line:max-line-length
      login: String(this.updateDataUser.get('login').value) === String(this.loginData) ? null : this.updateDataUser.get('login').value,
      img: this.imagen
    };
    if (data.firstName === null && data.lastName === null && data.email === null && data.login === null && data.img === null) {
      this.snackBar.open('Los datos existentes ya estan actualizados', null, {
        duration: 4000
      });
    } else {
      this.httpService.put(LandingConstants.UPDATE_USER_BY_ID_URI(this.httpService.userId), data).subscribe(res => {
        this.updateDataUser.reset();
        this.getInfoDb();
        this.httpService.userEmitter.emit(this.src);
